...
// let newa = a.split(' '); //không nên đặt tên ntn nhé, b nên đặt new_arr hoặc newArr (sẽ ảnh hưởng đến đọc code sau này)
// console.log(newa);
...
// let shopr = [shop[0]]; //B đặt tên biến 'shopr' và 'shop' giống nhau quá, người khác đọc code của b dễ bị nhầm và khó phân biệt
//
// while (true)
// {
//     let ui = prompt('Hi there, welcome to shop admin chanel, what do you want (C,R,U,D)?').toUpperCase(); //B nên khởi tạo 'ui' ngoài vòng lặp trước, chi nên gán đè giá trị
//
//     if (ui == 'R')
//     {
//         for (let a = 1; a <= shop.length - 1; a++)
//         {
//             shopr[a] = a + '. ' + shop[a];
//         }
//
...
//     else if (ui == 'U')
//     {
//         let uiu = Number(prompt('Enter the position you want to update')); //B cần đặt tên biến mang đúng ý nghĩa của nó (VD: updated_pos,...)
//
//         if (uiu >= 0 && uiu <= shop.length - 1 && Math.floor(uiu) == uiu) //chỉ cho ng dùng nhập số thứ tự từ '1' vì nhập '0' không thay đổi đc element nào
//         {
...
//         else
//         {
//             alert('The position you entered does not exist')
//         }
//     }
//     else if (ui == 'D')
//     {
...
//
//         if (uid >= 0 && uid <= shop.length - 1 && Math.floor(uid) == uid) //chỉ xóa được từ phần tử '1'
//         {
//             shop.splice(uid, 1);
//             console.log(shop)
//             alert('Done');
...
// else
// {
//     alert(`${num} is Found in my array at index ${arr.indexOf(num)}`) //B có thể chỉ cần áp dùng hàm indexOf() để vừa check element có tồn tại ko, vừa in ra vị trí
// }

// //8.1 Create an array to represent the sizes of your flock, and log all of your flock size, expected screen output:

// let sheep = [100, 101, 102];
...
//
// let nsheep = sheep.map(function (value, index)
// {
//     return (value += 50);
// });
// sheep = nsheep;
